...
to move-bacteria
  ask bacteria [
    ...
    fd (random 0.1) + 0.05
    if random-float 1 < (die-chance * ((count bacteria) / crowding-factor)) [die]
    if random-float 1 < (divide-chance * (1 - (pumps / 32.0)))
    [
      hatch-bacteria 1
      [
...
to move-antibiotics
  ask antibiotics [
    ...
    fd speed
    ask up-to-n-of 1 bacteria in-radius 0.5
    [
      let killed false
      if random-float 1 < (absorbtion-rate * (1 - (pumps / 12.0)))
      [
        set killed true
        antibiotic-kill
        die
      ]
      if killed [ask myself [die]]
    ]
  ]
end
...
to dose [g n]
  ...
  create-antibiotics n [
    set ycor max-pycor - 0.5
    set xcor random-float (max-pxcor - 1.0) * ((- 1) ^ random 2)
    set shape "circle"
    set size 0.3
    set color red
...
to do-dosing
  if auto-dose? and not (limit-doses? and (doses = num-doses))
  [
    if (ticks - (dose-delay * ticks-per-hour)) >= 0
    [
      if ((ticks - (dose-delay * ticks-per-hour)) mod (dose-frequency * ticks-per-hour)) = 0
      [
        if (random 100) > dose-skip-chance
        [
          dose 1 dose-size
          set doses doses + 1
        ]
...
"" ""
PENS
"1 Pump" 1.0 0 -16777216 true "" "plot count bacteria with [pumps = 1]"
"2 Pumps" 1.0 0 -7500403 true "" "plot count bacteria with [pumps = 2]"
"3 Pumps" 1.0 0 -2674135 true "" "plot count bacteria with [pumps = 3]"
"4 Pumps" 1.0 0 -955883 true "" "plot count bacteria with [pumps = 4]"
"5 Pumps" 1.0 0 -6459832 true "" "plot count bacteria with [pumps = 5]"
"6 Pumps" 1.0 0 -1184463 true "" "plot count bacteria with [pumps = 6]"
"7 Pumps" 1.0 0 -10899396 true "" "plot count bacteria with [pumps = 7]"
"8 Pumps" 1.0 0 -13840069 true "" "plot count bacteria with [pumps = 8]"
"Total" 1.0 0 -14835848 true "" "plot count bacteria"
...
"" ""
PENS
"Pumps" 1.0 1 -16777216 true "" "histogram [pumps] of bacteria"
